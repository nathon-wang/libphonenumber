...
var getNumberType = function (number, countryCode, callback) {
    var error = null;
    var result = null;

    try {
        var validNumber = validateNumber(number, countryCode);
        var phoneNumber = new phonenumbers.PhoneNumber();
        phoneNumber.setCountryCode(countryCode);
        phoneNumber.setNationalNumber(validNumber);
        result = phoneUtil.getNumberType(phoneNumber);
    }
    catch (e) {
        error = e;
    }
...
var isPossibleMobile = function (number, countryCode, callback) {
    var error = null;
    var result = null;

    try {
        var phoneType = getNumberType(number, countryCode);
        result = phoneType === 1 || phoneType === 2;
    }
    catch (e) {
        error = e;
    }
